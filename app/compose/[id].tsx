...
export default function ComposeDetailsScreen() {
  ...
  const fetchLogs = async () => {
    try {
      const response = await api.getComposeLogs(id);
      setLogs(response.logs || 'No stack logs available.');
    } catch {
      setLogs('Failed to retrieve compose stack logs.');
    }
  };

  const openLogsDrawer = () => {
    Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
    setLogs('Connecting to stack logs...');
  ...
  const handleAction = async (action: 'stop' | 'deploy') => {
    setActionLoading(true);
    Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Medium);

    try {
      if (action === 'deploy') {
        await api.redeployCompose(id);
        setCurrentStatus('running');
      } else if (action === 'stop') {
        await api.stopCompose(id);
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]} edges={['top', 'left', 'right']}>
      ...
      <View style={styles.header}>
        ...
        <View style={{ flex: 1 }}>
          <Text style={[styles.headerSubTitle, { color: colors.textSecondary }]}>{projectName} • COMPOSE</Text>
      ...
      <ScrollView style={styles.scroll}>
        ...
        <View style={[styles.card, styles.statusCard, { backgroundColor: colors.card, borderColor: colors.border }]}>
          <Text style={[styles.cardLabel, { color: colors.textSecondary }]}>Stack Lifecycle State</Text>
        ...
        {actionLoading ? (
          ...
          <View style={styles.controlGrid}>
            ...
            <TouchableOpacity style={[styles.controlBtn, styles.deployBtn, { backgroundColor: colors.activeTint, borderColor: colors.activeTint }]} onPress={() => handleAction('deploy')}>
              <Ionicons name="cloud-upload" size={18} color={colors.background} style={{ marginRight: 6 }} />
              <Text style={[styles.btnText, { color: colors.background }]}>Redeploy</Text>
            </TouchableOpacity>
          </View>
        )}
        ...
        >
          <View style={styles.logsRowLeft}>
            ...
            <View>
              <Text style={[styles.logsTitle, { color: colors.text }]}>View Stack Logs</Text>
              <Text style={[styles.logsSubtitle, { color: colors.textSecondary }]}>Inspect standard compose stdout/stderr logs</Text>
            </View>
          </View>
        ...
        <TouchableOpacity 
          ...
          onPress={() => {
            ...
            router.push({ 
              pathname: '/domains/[id]', 
              params: { id, type: 'compose', name, projectName }
            });
          }}
        >
          <View style={styles.logsRowLeft}>
        ...
        >
          <View style={styles.logsRowLeft}>
            ...
            <View>
              ...
              <Text style={[styles.logsSubtitle, { color: colors.textSecondary }]}>Configure S3 Docker volume backups</Text>
            </View>
          </View>
          <Ionicons name="chevron-forward" size={18} color={colors.textSecondary} />
        </TouchableOpacity>
      ...
      <BottomSheet ref={sheetRef} onClose={closeLogsDrawer}>
        <View style={styles.sheetContent}>
          ...
          <Text style={[styles.sheetSubtitle, { color: colors.textSecondary }]}>Compose stack stdout logs</Text>
...
const styles = StyleSheet.create({
  ...
  headerSubTitle: { fontSize: 12, fontWeight: '700', textTransform: 'uppercase', letterSpacing: 0.5 },
  headerTitle: { fontSize: 24, fontWeight: '800' },
  scroll: { paddingHorizontal: 16, paddingTop: 16 },
  card: { borderRadius: 12, borderWidth: 1, padding: 16, marginBottom: 14 },
  cardLabel: { fontSize: 11, fontWeight: '700', textTransform: 'uppercase', letterSpacing: 0.3, marginBottom: 8 },
  statusCard: { paddingVertical: 18 },
  statusRow: { flexDirection: 'row', alignItems: 'center' },
  statusDot: { width: 12, height: 12, borderRadius: 6, marginRight: 8 },
  statusText: { fontSize: 16, fontWeight: '700', letterSpacing: 0.2 },
  sectionTitle: { fontSize: 18, fontWeight: '700', marginBottom: 12, letterSpacing: -0.3 },
  loaderContainer: { height: 120, justifyContent: 'center', alignItems: 'center' },
  loaderText: { marginTop: 10, fontSize: 13 },
  controlGrid: { flexDirection: 'row', flexWrap: 'wrap', justifyContent: 'space-between' },
  controlBtn: { width: '48%', height: 52, borderRadius: 10, borderWidth: 1, flexDirection: 'row', justifyContent: 'center', alignItems: 'center', marginBottom: 12 },
  deployBtn: {},
  btnText: { fontWeight: '700', fontSize: 15 },
  logsRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' },
  logsRowLeft: { flexDirection: 'row', alignItems: 'center' },
  logsTitle: { fontSize: 15, fontWeight: '700' },
  logsSubtitle: { fontSize: 12, marginTop: 2, marginRight: 8, flexShrink: 1 },
  sheetContent: { flex: 1, paddingTop: 8 },
  sheetTitle: { fontSize: 24, fontWeight: '800', textAlign: 'center', marginBottom: 4 },
  sheetSubtitle: { fontSize: 13, textAlign: 'center', fontWeight: '600', marginBottom: 20 },
  terminalContainer: { flex: 1, backgroundColor: '#000000', borderRadius: 10, borderWidth: 1, borderColor: '#222222', padding: 12, marginBottom: 24 },
  terminalScroll: { paddingBottom: 20 },
  terminalText: { color: '#88ee88', fontFamily: 'monospace', fontSize: 12, lineHeight: 16 }
});
